...
def main():
    ...
    model = Grad_concat.GRAD(feats=args.feats, basic_conv=args.basic_conv, tail_conv=args.tail_conv)
    ...
    with tqdm(total=100) as t:
        ...
        for idx in range(0, 100):
            with h5py.File(
                ...
                sigma = sigma.reshape(1 * 1 * 1).expand(1, expand_H, expand_W)

                l_image = torch.cat((l_image, sigma), 0)
