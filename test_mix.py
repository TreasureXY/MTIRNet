...
'''
python3.6 test_concat.py --test_save --test_sigma 10 --test_result ./result_10 --gpu
python3.6 test_concat.py --test_sigma 10 --gpu --gpu_id 0 --test_save --test_l ../MyDIV2K/validation_L/10 --test_result ./test_result_10
'''
...
def main():
    ...
    model = Grad_mix.GRAD(feats=args.feats, basic_conv=args.basic_conv, tail_conv=args.tail_conv)
    ...
    with tqdm(total=100) as t:
        ...
        for idx in range(0, 100):
            with h5py.File(
                ...
                sigma_concat = sigma.reshape(1 * 1 * 1).expand(1, expand_H, expand_W)
                sigma_input = sigma.reshape(1 * 1 * 1).expand(args.feats, expand_H, expand_W)

                l_image = torch.cat((l_image, sigma_concat), 0)

                l_image = l_image.unsqueeze(0)
                h_image = h_image.unsqueeze(0)
                sigma_input = sigma_input.unsqueeze(0)

                l_image = l_image.to(device)
                h_image = h_image.to(device)
                sigma_input = sigma_input.to(device)

                with torch.no_grad():
                    output = model(l_image, sigma_input)
